...
## 스레드(Thread)는 무엇이며, 프로세스와 스레드의 차이는 무엇인지 설명해주세요.

#### 답변
스레드란, 프로세스가 할당 받은 자원을 이용하는 실행 단위를 의미한다.
프로세스와 스레드의 차이
프로세스는 강제로 종료될 경우 다른 프로세스에게 영향을 주지 않지만,
스레드는 프로세스 내 메모리 영역을 공유하기 때문에 하나에서 오류가 발생하면 다른 스레드가 모두 강제로 종료되는 차이점이 있다.
